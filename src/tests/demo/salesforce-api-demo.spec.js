...
const { TestRailUploader } = require('../../utils/testrail');
const TestCaseIdManager = require('../../utils/testrail/test-case-id-manager');
...
test.describe('Salesforce API Demo Tests', () => {
  ...
  test.beforeAll(async () => {
    ...
    try {
      const idManager = new TestCaseIdManager();
      testCaseIds = await idManager.assignUniqueTestCaseIds(2);
      console.log(`🔢 Assigned test case IDs: ${testCaseIds.join(', ')}`);
    } catch (error) {
      console.log('⚠️ Using fallback test case IDs');
